        ...
        for (var r = 0; r < records.length; r++) {
            ...
            if (!recording && now.isBetween(ra, rs)) {
                ...
                if (rec.command === 'mplayer') {
                    ...
                    commando = 'vlc --intf dummy --playlist-autostart --no-playlist-tree --run-time=' + length + ' --sout "#duplicate{dst=std{access=file,dst=' + conf.downloads + '/' + rec.filename.trim() + '_id-' + rec.id + '.' + rec.type + '}}" ' + rec.url.trim() + ' vlc://quit';
                } else { // streamripper
